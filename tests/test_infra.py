...
class TestDBFixture:
    ...
    def test_alerts_insert_and_query(self, db_conn):
        ...
        cursor.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            ("RED", "stock_drop", "005930.KS", "삼성전자 -5.3% 급락", -5.3, -5.0, "2026-03-25T15:30:00+09:00"),
        )
        db_conn.commit()
        cursor.execute("SELECT level, ticker FROM alerts WHERE event_type = 'stock_drop'")
        row = cursor.fetchone()
...
class TestSampleDataFixtures:
    ...
    def test_sample_prices_structure(self, sample_prices):
        ...
        required_keys = {"name", "ticker", "price", "prev_close", "change_pct", "volume", "market", "timestamp"}
    ...
    def test_sample_macro_structure(self, sample_macro):
        ...
        required_keys = {"name", "ticker", "value", "change_pct", "category", "timestamp"}
    ...
    def test_sample_news_structure(self, sample_news):
        ...
        required_keys = {"title", "summary", "source", "url", "published_at", "relevance_score", "tickers", "category"}
...
class TestConfig:
    ...
    def test_portfolio_required_fields(self):
        ...
        for item in PORTFOLIO:
            assert required.issubset(item.keys()), f"{item.get('name', '?')}에 필수 필드 누락"
